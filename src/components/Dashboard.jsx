...
const safeFetchJson = async (url, options = {}) => {
  try {
    ...
    if (!contentType.includes('application/json')) {
      console.warn(`[safeFetchJson] Rota "${url}" retornou HTTP ${res.status} (${contentType || 'sem content-type'}). Esperado JSON.`);
      return { ok: false, status: res.status, contentType, data: null, error: `Servidor retornou status ${res.status} (Não-JSON)` };
    }

    const data = await res.json();
    return { ok: res.ok, status: res.status, contentType, data, error: res.ok ? null : (data?.error || data?.message) };
  } catch (err) {
    console.warn(`[safeFetchJson] Erro ao comunicar com "${url}":`, err.message || err);
    return { ok: false, status: 0, contentType: '', data: null, error: err.message || 'Erro de rede' };
  }
};
...
export default function Dashboard({ session, profileDataProps }) {
  ...
  const fetchTenantSettings = async () => {
    try {
      ...
      const { ok, data: resData } = await safeFetchJson('/api/tenant/settings', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (ok && resData?.success && resData?.settings) {
        setTenantSettings(resData.settings);
      }
    } catch (err) {
      console.warn('Aviso ao buscar configurações do tenant:', err.message || err);
    }
  };
  ...
  const fetchTenantFaturas = async () => {
    ...
    try {
      ...
      const { ok, data: resData } = await safeFetchJson('/api/tenant/faturas', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      ...
      console.warn('Aviso ao buscar faturas:', err.message || err);
    } finally {
  ...
  const handleSaveTenantSettings = async (newSettings) => {
    ...
    try {
      ...
      const { ok, data: resData, error: fetchErr } = await safeFetchJson('/api/tenant/settings', {
        method: 'POST',
        headers: {
      ...
      if (ok && resData?.success && resData?.settings) {
        ...
        console.warn('Aviso ao salvar configurações:', fetchErr);
        alert('Recurso salvo localmente (servidor backend indisponível).');
      }
    } catch (err) {
  ...
  const handleSuperAdminCreateCompany = async (e) => {
    ...
    try {
      const { ok, data: result, error: fetchErr } = await safeFetchJson('/api/super-admin/create-company', {
        method: 'POST',
        headers: {
      ...
      if (!ok || !result) {
        throw new Error(fetchErr || 'Erro ao criar empresa');
      }
  ...
  const handleSuperAdminProvisionUser = async (e) => {
    ...
    try {
      const { ok, data: result, error: fetchErr } = await safeFetchJson('/api/super-admin/provision-user', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${session.access_token}`,
          'Content-Type': 'application/json'
      ...
      if (!ok || !result) {
        throw new Error(fetchErr || 'Erro ao provisionar usuário');
      }
  ...
  const handleSaveFiscalConfig = async (e) => {
    ...
    try {
      ...
      const { ok, data, error: fetchErr } = await safeFetchJson('/api/fiscal/configuracoes', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
      ...
      if (!ok || !data) {
        throw new Error(fetchErr || 'Falha ao salvar configurações fiscais.');
      }
  ...
  const handleEmitirNfePdv = async (reciboDados) => {
    ...
    try {
      ...
      const { ok, data, error: fetchErr } = await safeFetchJson('/api/fiscal/emitir-nfe-pdv', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
      ...
      if (!ok || !data) {
        throw new Error(fetchErr || 'Falha ao emitir nota fiscal.');
      }
  ...
  const handleSaveCatalogoProduto = async (e) => {
    ...
    const codigoLimpo = codigoBarras ? codigoBarras.trim() : null;
    const codigoBarrasFinal = (codigoLimpo && codigoLimpo !== "") ? codigoLimpo : null;
        ...
        try {
          if (!isEditing && hasInitialStockInput && catalogoFilialEstoque) {
            ...
            if (existingP) {
              ...
              const eanFisicoLimpo = data.codigo_barras ? String(data.codigo_barras).trim() : null;
              const insertObj = {
                empresa_id: targetEmpresaId,
  ...
  const handleBuscaMestreChange = (val) => {
    ...
    setEntradaBuscaMestre(termoLimpo);
    if (!termoLimpo.trim()) return;
    const cleanVal = termoLimpo.trim().toLowerCase();
  ...
  const handleBuscaMestreKeyDown = (e) => {
    if (e.key === 'Enter') {
      e.preventDefault();
      const val = entradaBuscaMestre.replace(/[\r\n]/g, '').trim();
      if (!val) return;
  ...
  const handleSalvarEstoqueFisico = async (e) => {
    ...
    try {
      ...
      const eanEntradaLimpo = entradaCodigoBarras ? entradaCodigoBarras.trim() : null;
      const eanMestreLimpo = selectedProdutoMestre?.codigo_barras ? selectedProdutoMestre.codigo_barras.trim() : null;
      const codigoBarrasCalculado = (eanEntradaLimpo && eanEntradaLimpo !== "")
        ? eanEntradaLimpo
  ...
  const handleTransfItemAdd = async (e) => {
    e.preventDefault();
    const term = transfImeiBusca.replace(/[\r\n]/g, '').trim();
    if (!term) return;
    ...
    try {
      ...
      // 2. Tentar buscar em Produtos (por Nome, Código de Barras ou SKU em OR condicional)
      const { data: prodData } = await supabase
        .from('produtos')
        .select('*')
        .eq('empresa_id', profile.empresa_id)
        .eq('filial_id', origemId)
        .or(`nome.ilike.%${term}%,codigo_barras.eq.${term},sku.eq.${term}`)
        .limit(1)
        .maybeSingle();
  ...
  const handleDebugBusca = async () => {
    const termo = (buscaEstoque || '').replace(/[\r\n]/g, '').trim();
    console.log("🔥 [DEBUG FORÇA BRUTA] Buscando termo exato:", termo);

    if (!termo) {
      alert("Por favor, digite ou bipe algo no campo de busca de estoque antes de clicar em DEBUG FORÇA BRUTA.");
      return;
    }

    // 1. Busca pura na tabela mestre de produtos (produtos_catalogo)
    let catData = null;
    let catError = null;
    try {
      const res = await supabase
        .from('produtos_catalogo')
        .select('*')
        .or(`nome.ilike.%${termo}%,codigo_barras.eq.${termo},sku.eq.${termo}`);
      catData = res.data;
      catError = res.error;
    } catch (err) {
      catError = err;
    }
    console.log("🔥 [DEBUG] Existe no Catálogo Mestre (produtos_catalogo)?", catData, catError);

    // 2. Busca pura na tabela de produtos física (produtos)
    let prodData = null;
    let prodError = null;
    try {
      const res = await supabase
    ...
    console.log("🔥 [DEBUG] Existe na Tabela de Produtos (produtos)?", prodData, prodError);

    // 3. Busca pura na tabela de IMEIs (imeis)
    let imeiData = null;
    let imeiError = null;
    try {
      const res = await supabase
        .from('imeis')
        .select('*, produtos(*)')
        .or(`imei.eq.${termo},imei.ilike.%${termo}%`);
      imeiData = res.data;
      imeiError = res.error;
    } catch (err) {
      imeiError = err;
    }
    console.log("🔥 [DEBUG] Existe na Tabela de IMEIs (imeis)?", imeiData, imeiError);

    alert(`🔥 [DEBUG FORÇA BRUTA EXECUTADO]\n\nResultados encontrados para "${termo}":\n- Catálogo Mestre (produtos_catalogo): ${catData?.length || 0} registro(s)\n- Tabela Produtos (produtos): ${prodData?.length || 0} registro(s)\n- Tabela IMEIs (imeis): ${imeiData?.length || 0} registro(s)\n\nAbra o Console (F12) para ver os dados brutos salvos no log.`);
  };
  ...
  const filteredProdutosEstoque = listaProdutosConsolidada.filter(p => {
    ...
    if (buscaEstoque) {
      const searchRaw = buscaEstoque.replace(/[\r\n]/g, '').trim();
      const searchLower = searchRaw.toLowerCase();
      const searchDigits = searchRaw.replace(/\D/g, '');

      const nameMatch = Boolean(p.nome && String(p.nome).toLowerCase().includes(searchLower));
      const skuMatch = Boolean(p.sku && String(p.sku).toLowerCase().includes(searchLower));
      const corMatch = Boolean(p.cor && String(p.cor).toLowerCase().includes(searchLower));
      const catMatchStr = Boolean(p.categoria && String(p.categoria).toLowerCase().includes(searchLower));
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        className={`w-full bg-black border rounded px-3 py-2 text-sm text-white outline-none transition-all cursor-pointer font-semibold ${isSameFilial ? 'border-red-500 focus:border-red-500' : 'border-[#222222] focus:border-[#6A0DAD]'
                          }`}
                      >
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                    ...
                    <div className="lg:col-span-5 bg-[#0A0A0A] border border-[#222222] rounded-lg p-6 flex flex-col shadow-xl">
                      ...
                      {provisionCompMessage.text && (
                        <div
                          className={`mb-4 p-3 rounded-md text-xs border flex items-start gap-2 ${provisionCompMessage.type === 'success'
                    ...
                    <div className="lg:col-span-7 bg-[#0A0A0A] border border-[#222222] rounded-lg p-6 flex flex-col shadow-xl">
                      ...
                      {provisionUserMessage.text && (
                        <div
                          className={`mb-4 p-3 rounded-md text-xs border flex items-start gap-2 ${provisionUserMessage.type === 'success'
                            ? 'bg-green-950/20 border-green-800 text-green-400'
                            : 'bg-red-950/20 border-red-800 text-red-400'
                            }`}
                        >
                              ...
                              <tr key={chamado.id} className="hover:bg-red-950/5 transition-colors">
                                ...
                                <td className="py-4">
                                  {chamado.status?.toLowerCase() === 'aberto' ? (
                                    ...
                                    <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-bold uppercase ${chamado.status?.toLowerCase() === 'em_analise' ? 'bg-yellow-950/30 text-yellow-400 border border-yellow-800/40' :
                              ...
                              <tr key={c.id} className={`transition-colors ${hasAtraso ? 'bg-red-950/10 hover:bg-red-950/15 border border-red-900/20' : 'hover:bg-purple-950/5'}`}>
                                ...
                                <td className="py-4">
                                  <select
                                    ...
                                    className={`border rounded px-2 py-1 text-xs font-bold outline-none cursor-pointer ${c.status_assinatura === 'BLOQUEADO'
                                ...
                                <td className="py-4">
                                  <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-bold ${c.status === 'ATIVO'
                                    ? 'bg-green-950/30 text-green-400 border border-green-800/40'
                                ...
                                <td className="py-4 text-right">
                                  <div className="flex items-center justify-end gap-2">
                                    <button
                                      ...
                                      className={`px-3 py-1.5 rounded text-xs font-bold transition-all border ${c.em_manutencao
                                    ...
                                    <button
                                      ...
                                      className={`px-3 py-1.5 rounded text-xs font-bold transition-all ${c.status === 'ATIVO'
                      ...
                      <div className="bg-[#0A0A0A] border border-[#222222] rounded-xl shadow-2xl w-full max-w-4xl flex flex-col max-h-[90vh] animate-fadeIn">
                        ...
                        <div className="flex border-b border-[#222222] px-6 bg-[#050505]">
                          <button
                            ...
                            className={`py-3 px-4 text-sm font-bold border-b-2 transition-all ${faturasActiveTab === 'config'
                          ...
                          <button
                            ...
                            className={`py-3 px-4 text-sm font-bold border-b-2 transition-all ${faturasActiveTab === 'historico'
                              ? 'border-[#6A0DAD] text-white'
                              : 'border-transparent text-gray-500 hover:text-gray-300'
                              }`}
                          >
                                      ...
                                      {companyFaturas.map(fatura => {
                                        ...
                                        return (
                                          <tr key={fatura.id} className="hover:bg-white/5">
                                            ...
                                            <td className="py-3">
                                              <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded text-xs font-bold uppercase ${fatura.status === 'PAGO' ? 'bg-green-500/10 text-green-400 border border-green-800/20' :
                                        ...
                                        return (
                                          <div key={idx} className="bg-[#0A0A0A] border border-[#222222] p-3 rounded-lg flex items-center justify-between gap-3">
                                            <div className="flex items-center gap-3 min-w-0">
                                              <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-extrabold border shrink-0 ${idx === 0 ? 'bg-yellow-950/60 text-yellow-400 border-yellow-700/60' :
                              ...
                              {filiais.map(f => {
                                ...
                                return (
                                  <div key={f.id} className={`flex justify-between items-center p-3 bg-black border rounded-md transition-colors ${editingFilial?.id === f.id ? 'border-[#6A0DAD] bg-[#6A0DAD]/10' : 'border-[#222222]'}`}>
                                    ...
                                    <div className="flex items-center gap-2">
                                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${isDepot ? 'bg-blue-950/20 text-blue-400 border border-blue-800/30' : 'bg-[#6A0DAD]/20 text-purple-400 border border-[#6A0DAD]/30'
                                        }`}>
                                ...
                                <div key={v.id} className="grid grid-cols-1 md:grid-cols-[1fr_auto] gap-4 items-center w-full p-4 border border-gray-800 bg-black rounded-lg hover:border-[#6A0DAD]/30 transition-all">
                                  ...
                                  <div className="flex flex-col gap-1">
                                    ...
                                    <div className="mt-1 flex gap-2">
                                      <span className={`px-2 py-0.5 rounded-[4px] text-[9px] font-bold whitespace-nowrap ${v.is_treinner
                          ...
                          <>
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                              ...
                              <div className="space-y-3">
                                ...
                                {/* Campo de Código de Barras (EAN / Barcode) - Exibido para todos os produtos */}
                                <div>
                                ...
                                {(tipoProduto === 'CELULAR' || tipoProduto === 'Celular') && (
                                  <div>
                                    <label className="block text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-1.5 font-sans">
                                ...
                                {(() => {
                                  ...
                                  if (vVenda > 0 || vCusto > 0) {
                                    ...
                                    return (
                                      <div className={`p-2.5 rounded-lg border text-xs flex items-center justify-between font-mono ${isLucroPositivo
                          ...
                          <div className="space-y-4 animate-fadeIn">
                            ...
                            <div className="flex border-b border-[#222222]/80 gap-6 mb-4">
                              <button
                                ...
                                className={`pb-2 text-xs font-bold transition-all border-b-2 uppercase tracking-wider ${torreSubTab === 'visao'
                              ...
                              <button
                                ...
                                className={`pb-2 text-xs font-bold transition-all border-b-2 uppercase tracking-wider ${torreSubTab === 'historico'
                                  ? 'text-white border-[#6A0DAD]'
                                  : 'text-gray-500 border-transparent hover:text-gray-300'
                                  }`}
                              >
                              ...
                              <>
                                <div className="mb-4 relative">
                                  ...
                                  <input
                                    ...
                                    placeholder="Buscar por IMEI, Código de Barras ou Modelo... (Bipe aqui)"
                                  />
                                </div>
                                    ...
                                    <table className="w-full text-left border-collapse whitespace-nowrap">
                                      ...
                                      <tbody className="divide-y divide-[#222222] bg-black">
                                        {torreData.length === 0 ? (
                                          ...
                                          torreData
                                            .filter(row => {
                                              if (!torreSearch || !torreSearch.trim()) return true;
                                              const s = String(torreSearch).toLowerCase().trim();
                                              const sDigits = s.replace(/\D/g, '');
                                              const isNumericQuery = sDigits.length >= 3 && (sDigits.length / s.length >= 0.7 || /^\d+$/.test(s));

                                              const rName = String(row.nome || '').toLowerCase();
                                              const rBarcode = String(row.codigo_barras || '').toLowerCase();
                                              const rBarcodeDigits = rBarcode.replace(/\D/g, '');
                                              const rSku = String(row.sku || '').toLowerCase();

                                              const nameMatch = rName.includes(s);
                                              const barcodeMatch = Boolean(
                                                (rBarcode && rBarcode.includes(s)) ||
                                                (isNumericQuery && rBarcodeDigits && rBarcodeDigits.includes(sDigits))
                                              );
                                              const skuMatch = Boolean(rSku && rSku.includes(s));

                                              const itemsMatch = (row.items || []).some(i => {
                                                if (!i) return false;
                                                const imeiStr = String(i.imei || '').toLowerCase();
                                                const imeiDigits = imeiStr.replace(/\D/g, '');
                                                const iBarcodeStr = String(i.codigo_barras || '').toLowerCase();
                                                const iBarcodeDigits = iBarcodeStr.replace(/\D/g, '');

                                                return imeiStr.includes(s) ||
                                                  iBarcodeStr.includes(s) ||
                                                  (isNumericQuery && imeiDigits && imeiDigits.includes(sDigits)) ||
                                                  (isNumericQuery && iBarcodeDigits && iBarcodeDigits.includes(sDigits));
                                              });

                                              return nameMatch || barcodeMatch || skuMatch || itemsMatch;
                                            })
                                            .map((row, idx) => {
                                              const isSearching = Boolean(torreSearch && torreSearch.trim().length > 0);
                                              const isExpanded = Boolean(torreExpandedRows[row.nome] || isSearching);
                                              return (
                                                      ...
                                                      <td colSpan={torreFiliais.length + 3} className="p-0 border-b border-[#222222]">
                                                        <div className="p-4 pl-10">
                                                          <table className="w-full text-xs text-left border border-[#222222] rounded-md overflow-hidden">
                                                            ...
                                                            <tbody className="divide-y divide-[#222222]">
                                                              {row.items.map(item => {
                                                                const searchStr = String(torreSearch || '').toLowerCase().trim();
                                                                const searchDigits = searchStr.replace(/\D/g, '');
                                                                const isNumSearch = searchDigits.length >= 3 && (searchDigits.length / searchStr.length >= 0.7 || /^\d+$/.test(searchStr));

                                                                const imeiVal = String(item.imei || '').toLowerCase();
                                                                const imeiDigits = imeiVal.replace(/\D/g, '');
                                                                const barcodeVal = String(item.codigo_barras || '').toLowerCase();
                                                                const barcodeDigits = barcodeVal.replace(/\D/g, '');

                                                                const isHighlighted = Boolean(
                                                                  searchStr && (
                                                                    imeiVal.includes(searchStr) ||
                                                                    barcodeVal.includes(searchStr) ||
                                                                    (isNumSearch && imeiDigits && imeiDigits.includes(searchDigits)) ||
                                                                    (isNumSearch && barcodeDigits && barcodeDigits.includes(searchDigits))
                                                                  )
                                                                );
                                                                return (
                          ...
                          <div className="space-y-4">
                            ...
                            <div className="flex items-center justify-between p-4 bg-black border border-[#222] rounded-lg">
                              ...
                              <button
                                ...
                                className={`w-12 h-6 flex items-center rounded-full p-1 transition-all duration-300 ${tenantSettings.enable_troca ? 'bg-[#6A0DAD] justify-end' : 'bg-gray-800 justify-start'
                                  }`}
                              >
                            ...
                            <div className="flex items-center justify-between p-4 bg-black border border-[#222] rounded-lg">
                              ...
                              <button
                                ...
                                disabled={isSavingSettings}
                                className={`w-12 h-6 flex items-center rounded-full p-1 transition-all duration-300 ${tenantSettings.enable_imei ? 'bg-[#6A0DAD] justify-end' : 'bg-gray-800 justify-start'
                                  }`}
                              >
                                <span className="bg-white w-4 h-4 rounded-full shadow-md"></span>
                              </button>
                            ...
                            <tbody className="divide-y divide-[#222222]/50">
                              {tenantFaturas.map((fatura) => (
                                <tr key={fatura.id} className="hover:bg-white/5 transition-colors">
                                  ...
                                  <td className="py-3">
                                    <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${fatura.status === 'PAGO'
                      ...
                      <div className="bg-[#0A0A0A] border border-[#222222] rounded-xl p-5 flex flex-col">
                        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4">
                          ...
                          <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto flex-wrap">
                            ...
                            <button
                              type="button"
                              onClick={handleDebugBusca}
                              className="bg-red-600 hover:bg-red-700 text-white font-bold text-xs px-3 py-2 rounded-md transition-all shadow-md flex items-center gap-1 shrink-0"
                              title="Forçar busca pura no Supabase sem filtros de loja/interface"
                            >
                              🔥 DEBUG FORÇA BRUTA
                            </button>
                            <select
              ...
              {loadingClienteHistorico ? (
                ...
                <div className="space-y-6">
                  ...
                  {(() => {
                    ...
                    return (
                      <div className={`p-4 rounded-xl border flex flex-col md:flex-row items-start md:items-center justify-between gap-3 animate-fadeIn ${isQuitandoEsteMes || isJáQuitado
                        ...
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                            <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider ${isQuitandoEsteMes || isJáQuitado ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/40' : 'bg-purple-500/20 text-purple-300 border border-purple-500/40'
                              }`}>
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 border-b border-[#222222] pb-3">
                      ...
                      <div className="flex gap-1.5 flex-wrap">
                        <button
                          ...
                          className={`px-2.5 py-1 rounded text-[10px] font-bold uppercase transition-all border ${selectedPromoType === 'upgrade'
                            ? 'bg-[#6A0DAD] text-white border-[#6A0DAD]'
                            : 'bg-black text-gray-400 border-[#222222] hover:text-white'
                        ...
                        <button
                          ...
                          className={`px-2.5 py-1 rounded text-[10px] font-bold uppercase transition-all border ${selectedPromoType === 'quitacao'
                            ? 'bg-[#6A0DAD] text-white border-[#6A0DAD]'
                            : 'bg-black text-gray-400 border-[#222222] hover:text-white'
                            } ${isGeneratingIA ? 'opacity-50 cursor-wait' : 'cursor-pointer'}`}
                        >
                        ...
                        <button
                          ...
                          className={`px-2.5 py-1 rounded text-[10px] font-bold uppercase transition-all border ${selectedPromoType === 'acessorios'
                            ? 'bg-[#6A0DAD] text-white border-[#6A0DAD]'
                            : 'bg-black text-gray-400 border-[#222222] hover:text-white'
                            } ${isGeneratingIA ? 'opacity-50 cursor-wait' : 'cursor-pointer'}`}
                        >
                        ...
                        {(() => {
                          ...
                          return (
                            <a
                              ...
                              className={`flex-1 sm:flex-none px-4 py-2 rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-1.5 text-white shadow-lg ${waLink ? 'bg-emerald-600 hover:bg-emerald-500 shadow-emerald-950/40 cursor-pointer' : 'bg-gray-800 opacity-50 cursor-not-allowed'
                                }`}
                            >
